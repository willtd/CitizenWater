...
static mraa_i2c_context i2c_init();
...
static atlsci_read_ret_t parse_atlsci_read(int sensor_type, uint8_t * led_string)
{
    ...
    char * cal_ret = malloc(CONDUCTIVITY_CAL_HIGH_SIZE);
    int counter = 0;
    ...
    {
	while(led_string[counter] != ',')
	{
		cal_ret[counter] = led_string[counter];
		++counter;
	}
	cal_ret[counter] = '\0';
	ret.data = cal_ret;
...
int atlsci_init()
{
   // mraa_init();
    ...
    i2c_context = i2c_init();
    if (i2c_context == NULL)
    {
        DEBUG("atlsci_init(): Failed to initialize I2C\n");
        return ATLSCI_FAILURE;
    }
